...
while True:
    ...
    try:
        ...
        for a in soup.find_all("a", href=True):
            href = a["href"]
            # Correction majeure : on passe tout en minuscules (.lower()) pour éviter le piège TheoLap vs theolap
        ...
        if not page_links:
            print("🏁 Plus aucun vol trouvé sur cette page. Fin du scan.")
            break
...
for url in flight_links:
    # On crée un nom de fichier unique et propre basé directement sur la fin de l'URL du vol
    # Exemple : .../detail:theolap/24.05.2025/11:30 -> theolap_24.05.2025_11-30.igc
    ...
    if not os.path.exists(filepath):
        ...
        try:
            ...
            # Vérification rudimentaire du format IGC valide
            if r.status_code == 200 and ("A" in r.text[:5] or "B" in r.text[:200] or "XCONTEST" in r.text[:100]):
                with open(filepath, "w", encoding="utf-8", errors="ignore") as f:
                    f.write(r.text)
                vols_telecharges += 1
                time.sleep(1.5) # Pause de sécurité pour XContest
            else:
                print(f"⚠️ Fichier non disponible ou format incorrect pour ce vol.")
        except Exception as e:
            print(f"❌ Erreur de téléchargement : {e}")

print(f"✅ Téléchargement terminé. {vols_telecharges} nouvelles traces ajoutées au dossier.")

# 3. Lecture et extraction des coordonnées GPS
vols_par_annee = {}
for filename in os.listdir(TRACKS_DIR):
    if not filename.endswith(".igc"): 
        continue
        
    # Extraction de l'année (ex: theolap_24.05.2025_11-30.igc -> 2025)
    try:
    ...
    with open(filepath, "r", errors="ignore") as f:
        for line in f:
            if line.startswith("B"): # Point GPS valide dans un fichier IGC
                try:
...
for annee in sorted(vols_par_annee.keys(), reverse=True):
    ...
    for trace in vols_par_annee[annee]:
        # Opacity=0.25 et weight=2 pour l'effet de surbrillance/heatmap
        folium.PolyLine(trace, color="red", weight=2, opacity=0.25).add_to(group)
    group.add_to(m)

# Ajout du panneau de filtres des années
